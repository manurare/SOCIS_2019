...
class Net(object):
  ...
  def conditioning_network(self, lr_images, size_hr_lr):
    """
    Args:[-0.5, 0.5]
  ...
  def construct_net(self, hr_images, lr_images):
    ...
    size_hr_lr = [int(hr_images.shape[1]), int(lr_images.shape[1])]
    self.prior_logits = self.prior_network(hr_images)
    self.conditioning_logits = self.conditioning_network(lr_images, size_hr_lr)
